...
from maskrcnn_benchmark.solver import make_mtm_ship_optimizer
from maskrcnn_benchmark.solver import validate_optimizer_parameters
...
def train(cfg, local_rank, distributed, logger):
    ...
    mtm_ship_optimizer = make_mtm_ship_optimizer(cfg, model)
    validate_optimizer_parameters(model, optimizer, mtm_ship_optimizer)
    ...
    if checkpointer.has_checkpoint():
        ...
        mtm_ship_optimizer_state = extra_checkpoint_data.pop(
            "mtm_ship_optimizer", None
        )
        if mtm_ship_optimizer is not None and mtm_ship_optimizer_state is not None:
            mtm_ship_optimizer.load_state_dict(mtm_ship_optimizer_state)
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        optimizer.zero_grad()
        if mtm_ship_optimizer is not None:
            mtm_ship_optimizer.zero_grad()
