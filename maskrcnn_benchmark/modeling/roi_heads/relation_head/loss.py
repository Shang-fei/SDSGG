...
class PrimitiveVAEReconstructionLoss(nn.Module):
    """
    Reconstruction loss for the primitive semantic VAE.

    The VAE reconstructs predicate primitive composition, not image, union, or
    entity appearance features. Given reconstruction logits [num_rel, K] and
    relation labels, this loss gathers each predicate's primitive prior and
    compares it with the decoded primitive logits.
    """

    def __init__(self, loss_type="bce", ignore_background=True, primitive_channel=2):
        super(PrimitiveVAEReconstructionLoss, self).__init__()
        if loss_type not in ("bce", "mse"):
            raise ValueError("loss_type must be 'bce' or 'mse', got {}".format(loss_type))
        self.loss_type = loss_type
        self.ignore_background = ignore_background
        self.primitive_channel = primitive_channel

    def _normalize_prior(self, predicate_primitive_prior):
        if predicate_primitive_prior.dim() == 3:
            return predicate_primitive_prior[:, self.primitive_channel, :]
        if predicate_primitive_prior.dim() == 2:
            return predicate_primitive_prior
        raise ValueError(
            "predicate_primitive_prior must be [num_pred, K] or [num_pred, M, K], got {}".format(
                tuple(predicate_primitive_prior.shape)
            )
        )

    def forward(self, recon_logits, rel_labels, predicate_primitive_prior):
        if isinstance(rel_labels, (list, tuple)):
            rel_labels = cat(rel_labels, dim=0)
        rel_labels = rel_labels.view(-1).long().to(recon_logits.device)

        if self.ignore_background:
            keep = rel_labels > 0
            recon_logits = recon_logits[keep]
            rel_labels = rel_labels[keep]

        if rel_labels.numel() == 0:
            return recon_logits.sum() * 0.0

        primitive_prior = self._normalize_prior(predicate_primitive_prior)
        primitive_prior = primitive_prior.to(device=recon_logits.device, dtype=recon_logits.dtype)
        targets = primitive_prior[rel_labels]

        if targets.size(-1) != recon_logits.size(-1):
            raise ValueError(
                "Primitive target dimension {} does not match reconstruction dimension {}".format(
                    targets.size(-1), recon_logits.size(-1)
                )
            )

        if self.loss_type == "bce":
            return F.binary_cross_entropy_with_logits(recon_logits, targets, reduction="mean")
        return F.mse_loss(torch.sigmoid(recon_logits), targets, reduction="mean")


def primitive_vae_kl_loss(mu, logvar, prior_mu, prior_logvar, rel_labels=None,
                          ignore_background=True, reduction="mean"):
    """
    KL(q(z|x) || p_y(z)) for diagonal Gaussian primitive distributions.

    This helper is available for the later KL stage. The first VAE stage can
    use only PrimitiveVAEReconstructionLoss.
    """
    if rel_labels is not None:
        if isinstance(rel_labels, (list, tuple)):
            rel_labels = cat(rel_labels, dim=0)
        rel_labels = rel_labels.view(-1).long().to(mu.device)
        ...
        if rel_labels.numel() == 0:
            return mu.sum() * 0.0
        prior_mu = prior_mu.to(device=mu.device, dtype=mu.dtype)[rel_labels]
        prior_logvar = prior_logvar.to(device=mu.device, dtype=mu.dtype)[rel_labels]
    else:
        prior_mu = prior_mu.to(device=mu.device, dtype=mu.dtype)
        prior_logvar = prior_logvar.to(device=mu.device, dtype=mu.dtype)
